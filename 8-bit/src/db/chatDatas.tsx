// mockData.js

  const userLists = [
    { sender: "이자인", content: "혹시 스나이퍼팩토리 교육과정 어떠셨나요?"},
    { sender: "신유뎀", content: "프로젝트 팀원 구합니다!"},
    { sender: "김스팩", content: "사이드 프로젝트 같이 하실래요?"},
    { sender: "김스팩", content: "포트폴리오 정말 멋지네요! 더 좋은 작품 기대하겠습니다!!"},
    { sender: "김스팩", content: "포트폴리오 정말 멋지네요! 더 좋은 작품 기대하겠습니다!!"},
    { sender: "김스팩", content: "포트폴리오 정말 멋지네요! 더 좋은 작품 기대하겠습니다!!"},
    { sender: "김스팩", content: "포트폴리오 정말 멋지네요! 더 좋은 작품 기대하겠습니다!!"},
    { sender: "김스팩", content: "포트폴리오 정말 멋지네요! 더 좋은 작품 기대하겠습니다!!"},
    { sender: "김스팩", content: "포트폴리오 정말 멋지네요! 더 좋은 작품 기대하겠습니다!!" },
  ]
  const mockMessages = [
    { sender: "이자인", content: "안녕하세요. 이번에 올리신 포트폴리오 정말 잘 봤습니다~!"},
    { sender: "이자인", content: "혹시 부트캠프 교육과정 중에 만드신 걸까요?"},
    { sender: "최준생", content: "안녕하세요! 칭찬 감사합니다!ㅠㅠ"},
    { sender: "최준생", content: "아, 부트캠프는 아니고 스나이퍼 팩토리 인턴형 프로그램에 참여했을 때 제작했던 프로젝트 입니다."},
    { sender: "이자인", content: "오! 스나이퍼 팩토리는 처음 들어보네요!"},
    { sender: "이자인", content: "찾아보니까 얼마전에 2기도 모집했었네요!"},
    { sender: "이자인", content: "스나이퍼 팩토리 교육과정 추천하시나요?"},
  ];
  
  export {userLists, mockMessages};